...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInAsGuest = async () => {
    setLoading(true);
    try {
      saveSession(await authApi.guest());
    } finally {
      setLoading(false);
    }
  };
  ...
  const signOut = () => {
    ...
    localStorage.removeItem("datadoctor_session");
    setSession(null);
  };

  useEffect(() => {
    const handler = () => signOut();
    window.addEventListener("datadoctor:unauthorized", handler);
    return () => window.removeEventListener("datadoctor:unauthorized", handler);
  }, []);

  return (
    <AuthContext.Provider value={{ session, loading, signIn, signUp, signOut, signInAsGuest }}>
      {children}
